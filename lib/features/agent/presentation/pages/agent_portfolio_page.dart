...
class _AgentPortfolioPageState extends State<AgentPortfolioPage> {
  ...
  Future<void> _load() async {
    ...
    if (resp['success'] == true && resp['data'] != null) {
      ...
      setState(() { _items = props; _loading = false; });
    } else {
      setState(() { _error = resp['error'] ?? 'Error cargando portafolio'; _loading = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        elevation: 0,
        title: const Text('Portafolio', style: TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.w600)),
        actions: [IconButton(onPressed: _load, icon: const Icon(Icons.refresh, color: Colors.black))],
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.pushNamed(context, '/add-property').then((_) => _load()),
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Text('Bs. ${p.price.toStringAsFixed(0)}/mes · ${p.size.toInt()}m²'),
                          trailing: Icon(p.isActive ? Icons.toggle_on : Icons.toggle_off, color: p.isActive ? Colors.green : Colors.grey),
                          onTap: () {},
                        ),
                      );
                    },
