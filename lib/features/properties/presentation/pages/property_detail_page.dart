...
import 'package:habitto/features/properties/domain/entities/property.dart' as domain;
import 'package:habitto/features/properties/domain/entities/photo.dart' as domain_photo;
...
class _PropertyDetailPageState extends State<PropertyDetailPage> {
            ...
            : _property == null
                ...
                : ListView(
                    ...
                    children: [
                            ...
                            return Container(
                              ...
                              child: ClipRRect(
                                ...
                                child: _photos.isEmpty
                                    ? _placeholderImage()
                                    : _networkImage(_photos[i], fit: BoxFit.cover),
                              ),
                            );
                          },
                      ...
                      Text(
                        _property!.address.isNotEmpty ? _property!.address : 'Propiedad',
                        style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700),
                      ),
                      const SizedBox(height: 6),
                      Text(
                        _property!.price > 0 ? 'Bs. ${_property!.price.toStringAsFixed(0)}/mes' : '—',
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              spacing: 12,
                              runSpacing: 12,
                              children: _property!.amenities.map((_) => _amenityChip()).toList(),
                            ),
                          ],
                        ),
                      ),
                      ...
                      Row(
                        children: [
                          Expanded(child: _primaryButton('Swipe para Match', cs.primary, Colors.black, () {})),
                        ],
                      ),
                      const SizedBox(height: 10),
                      Row(
                        children: [
                          Expanded(child: _secondaryButton('Solicitar Roomie', () {})),
                          const SizedBox(width: 12),
                          Expanded(child: _secondaryButton('Agendar Vista', () {})),
                        ],
                      ),
                      const SizedBox(height: 24),
                      const Text('Reseñas', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 12),
                      _reviewTile('Juan Perez', '¡El departamento es increíble! Muy bien ubicado y con todas las comodidades. 100% recomendado.'),
                      const SizedBox(height: 10),
                      _reviewTile('María García', 'Buena ubicación, aunque un poco ruidoso por la noche. El anfitrión fue muy amable.'),
                    ],
                  ),
      ),
    );
  ...
  Widget _placeholderImage() {
    return Container(color: Colors.grey.shade300, alignment: Alignment.center, child: const Icon(Icons.image, size: 36));
  }
  ...
  Widget _amenityChip() {
    return Container(
      ...
      child: const Row(children: [Icon(Icons.check_circle, size: 16), SizedBox(width: 6), Text('Amenidad')]),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: bg,
          borderRadius: BorderRadius.circular(16),
          boxShadow: [BoxShadow(color: bg.withOpacity(0.35), blurRadius: 12, offset: const Offset(0, 8))],
        ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(children: const [Icon(Icons.star, size: 16, color: Colors.amber), Icon(Icons.star, size: 16, color: Colors.amber), Icon(Icons.star, size: 16, color: Colors.amber), Icon(Icons.star, size: 16, color: Colors.amber), Icon(Icons.star_half, size: 16, color: Colors.amber)]),
                const SizedBox(height: 6),
                Text(text, style: TextStyle(color: Colors.black.withOpacity(0.8))),
              ],
            ),
          ),
        ],
